...
**Two coordinated release-flow gates plus a rebuild-elimination step: `release.ps1` runs a preflight `workflow_dispatch` build and refuses to create the tag if any binary fails; the tag-triggered path now downloads those pre-built artifacts via `actions/download-artifact` cross-run instead of rebuilding, so tag push time drops from ~15 min to ~2 min; and `publish-pypi` cannot fire without a successful preflight run for the exact commit SHA. Together these guarantee tags, PyPI wheels, and the GitHub Release page stay in lockstep — no more half-released 1.1.8/1.1.9-style outcomes, and no wasted CI. The 1.1.8–1.1.10 attempts to ship a third `-glibc2.28-` Linux binary from a `manylinux_2_28` container are reverted; RHEL/Rocky/Alma 8+9 and Amazon Linux 2023 users install via `pip install coverity-metrics` from PyPI instead. Docs: `INSTALL.md` now has an explicit `python -m venv` install recipe and a full air-gapped install walkthrough (wheelhouse build → transfer → `--no-index` install) for hosts with no PyPI access. No source, CLI, config, or dependency changes to the package itself.**

#### Added

##### 📖 `INSTALL.md` now documents a plain `python -m venv` install recipe
- **Symptom** — Users on locked-down machines who prefer managing their own virtual environment (rather than `pip install --user` or `pipx`) had to piece together the venv commands themselves. The existing "Installing without admin or root rights" section covered `--user` install and `pipx` but not a standalone `python -m venv` walkthrough.
- **Fix** — New subsection in [INSTALL.md](INSTALL.md) between "From PyPI" and "Installing without admin or root rights": **"Into a Python virtual environment (recommended for keeping installs isolated)"**. Provides copy-paste-ready snippets for Linux/macOS bash/zsh, Windows PowerShell, and Windows cmd.exe, each covering the full lifecycle: `python -m venv .venv` → activate → `pip install --upgrade pip` → `pip install coverity-metrics` → CLI smoke-test → `deactivate`. Windows PowerShell snippet notes the one-time `Set-ExecutionPolicy -Scope Process -ExecutionPolicy RemoteSigned` fix for activation errors. Explains that `.venv/` is fully self-contained (delete the folder to wipe the install), and points at `pipx` for readers who want the same isolation without hand-managing venvs.
- **How to recover** — No action needed for existing installs. Fresh installs can follow the new section directly.

##### 📖 `INSTALL.md` now covers air-gapped install (no internet on the target machine)
- **Symptom** — With the `-glibc2.28-` standalone binary removed this release, RHEL/Rocky/Alma 8+9 users are directed at `pip install coverity-metrics` — but many of those hosts sit behind an air-gap with no route to PyPI. The prior INSTALL.md had no guidance for that scenario; users had to derive the `pip download` → transfer → `pip install --no-index` recipe themselves and typically tripped on platform / ABI tag mismatches on the way.
- **Fix** — New "Air-gapped install (no internet on the target machine)" section in [INSTALL.md](INSTALL.md) between the venv recipe and "Installing without admin or root rights". Documents the three-step wheelhouse workflow end-to-end:
  1. **On an internet-connected machine**: `python -m pip download --dest wheelhouse --only-binary=:all: coverity-metrics pip setuptools wheel`. Two flavors provided — Flavor A when the connected machine is the same OS + Python as the target (simplest), Flavor B when they differ (pins `--python-version`, `--platform`, `--implementation`, `--abi`). Coverage table of standard `--platform` tags for Linux RHEL 8+ (`manylinux2014_x86_64`), newer Linux (`manylinux_2_28_x86_64` / `manylinux_2_34_x86_64`), Windows (`win_amd64`), macOS ARM (`macosx_11_0_arm64`), macOS Intel (`macosx_10_9_x86_64`), matched to CPython version tags (`cp311`/`cp312`/`cp313`/`cp314`).
  2. **Bundle**: `tar czf coverity-metrics-airgapped-<version>.tar.gz wheelhouse/` on Linux/macOS or `Compress-Archive` on Windows.
  3. **On the target**: extract, create a venv, `pip install --no-index --find-links wheelhouse --upgrade pip setuptools wheel`, then `pip install --no-index --find-links wheelhouse coverity-metrics`. `--no-index` disables PyPI lookups outright; the whole install runs without any network calls.

  Includes troubleshooting for the three common failure modes: (a) `Could not find a version that satisfies the requirement <pkg>` during download → wrong `--platform` / `--python-version` / `--abi` combo (fix: use widest-compat `manylinux2014_x86_64` or bump target Python); (b) same error during install on the target → wheelhouse ABI doesn't match target's `sysconfig.get_platform()` / `sysconfig.get_python_version()` (fix: re-download with matching tags); (c) `subprocess-exited-with-error` compiling pandas/numpy → sdist slipped in, add `--only-binary=:all:` back. Plus notes on `pipx --pip-args="--no-index --find-links wheelhouse"` compatibility, in-place upgrades, and the ~100 MB typical bundle size (pandas + numpy dominate).
- **How to recover** — No action needed for existing online installs. Air-gapped hosts follow the new section directly; no CI or workflow changes are required, and no new artefacts are attached to the release page — the recipe uses stock `pip download` against PyPI.

#### Changed

##### 🔒 Release flow refuses to tag / publish until every binary is built — and the tag push no longer rebuilds the binaries
- **Symptom** — The 1.1.8 and 1.1.9 releases both landed on PyPI without the matching Linux binaries on the GitHub Release page, because `publish-pypi` ran independently of the binary-build jobs and the release-script tagged locally before knowing whether the workflow would succeed. Users pulling the tag from GitHub Releases got a Windows exe and nothing else; users pulling from PyPI got a working wheel but no drop-in binary. On top of that, once the gate WAS added, tag push still spent ~15 min rebuilding binaries the preflight had already produced 30 seconds earlier — pure waste, and each rebuild was one more chance for a flaky CI cycle to lock up an otherwise good release.
- **Root cause** — Three independent problems:
  1. In the pre-1.1.10 `.github/workflows/build-binaries.yml`, `publish-pypi` had `if: startsWith(github.ref, 'refs/tags/v')` but *no* `needs:` line, so it ran in parallel to every build job. A tag push published to PyPI even if binaries failed later in the same run.
  2. In the pre-1.1.10 `release.ps1`, `git tag` + `git push` ran immediately after the version-bump commit was pushed. The tag existed on origin before the release workflow finished, so a failed binary build couldn't roll the tag back.
  3. After (1) and (2) were fixed with `needs: [build]` and a client-side preflight, the workflow ran `build` twice for every release: once via `workflow_dispatch` from the preflight, once again when the tag was pushed. Same binaries, same OpenSSL/zlib source-builds, same PyInstaller time — all thrown away.
- **Fix** — Three coordinated changes:
  - **Skip the tag-triggered rebuild.** The `build` matrix job is now gated on `if: github.event_name == 'workflow_dispatch'` so pushing a tag does NOT re-run it. A new `find-preflight-run` job runs on tag push, calls `gh run list --workflow=build-binaries.yml --status=success --json databaseId,headSha --jq "[.[] | select(.headSha==\"$SHA\")][0].databaseId"` to locate the preflight run that produced artifacts for the tag's commit, and hands its `run_id` to the `release` job. `release` then uses `actions/download-artifact@v5` with `run-id:` + `github-token:` for a cross-run download of the pre-built binaries. `publish-pypi` still runs on tag push but only builds the sdist/wheel (~30 s) — it no longer needs `build`. Added `permissions: actions: read` at workflow scope so the cross-run API has the token scopes it needs.
  - **Half-release protection preserved.** `find-preflight-run` fails loudly if no successful preflight run exists for the tag's commit SHA (typical trigger: someone pushed a tag by hand, bypassing `release.ps1`), so `publish-pypi` and `release` — which both `needs: [find-preflight-run]` — never fire. Even manual tag pushes can't produce a half-release; the only way to get to PyPI publish is via a green preflight run whose artifacts still exist (30-day retention).
  - **Client-side preflight** unchanged from earlier in 1.1.10: `release.ps1` runs `git push $Branch` → `gh workflow run build-binaries.yml --ref $Branch` → `gh run watch --exit-status <newest-run>` → only then `git tag` + `git push $Remote $tag`. `-SkipPreflightCI` is the escape hatch; `-DryRun` narrates every preflight step without side effects.
- **Net effect** — Tag push time drops from ~15 min (build + publish + release) to ~2 min (find-preflight + publish + release), and no binaries are ever produced twice for a single release. The only per-release "duplication" left is the PyPI sdist/wheel build inside `publish-pypi`, which is inherently cheap and can't be sourced from a workflow_dispatch run because the wheel metadata is version-tag-derived.
- **How to recover** — Upgrade `release.ps1` (comes with 1.1.10) and make sure the [GitHub CLI](https://cli.github.com/) is installed and authenticated (`gh auth login`) on the machine you cut releases from. Every future `./release.ps1 -NewVersion X.Y.Z` will refuse to tag until all three binary artefacts (Windows, linux, linux-glibc2.35) have built successfully on the just-pushed commit, and the tag push itself will only reuse those artefacts rather than rebuild.

#### Removed

##### 🗑️ `coverity-metrics-linux-glibc2.28-<version>` binary reverted; RHEL/Rocky/Alma 8+9 users now `pip install` from PyPI
- **Symptom** — The `-glibc2.28-` binary designed in 1.1.8 was never actually attached to any release page (1.1.8, 1.1.9, and 1.1.10 preflights all failed the container-build job before it could produce an artefact). The failure moved every iteration: `ldd | head` SIGPIPE → `Can't locate IPC/Cmd.pm` → `Can't locate Time/Piece.pm` → the OpenSSL 3.5.7 source-build ran clean but `LD_LIBRARY_PATH` couldn't override `_ssl.so`'s `libssl.so.1.1` SONAME with our `libssl.so.3`, so `ssl.OPENSSL_VERSION` stayed on 1.1.1k FIPS. Each fix uncovered the next problem; each failed preflight also cost one CI cycle.
- **Root cause** — Structural, not fixable in-workflow. The `quay.io/pypa/manylinux_2_28_x86_64` image's pre-built CPython 3.14 (which we have to use — `actions/setup-python` inside the container would install a Python compiled against a newer glibc and defeat the whole point of using the container) is linked against `libssl.so.1.1` from AlmaLinux 8's system OpenSSL 1.1.1k. `ld.so` will only ever look for a candidate whose SONAME matches the DT_NEEDED entry, so no combination of `LD_LIBRARY_PATH`, `perl-core`, or venv-based bootstrap can retarget an already-compiled `_ssl.so` at a fresh `libssl.so.3`. Shipping the binary anyway would have baked OpenSSL 1.1.1k (EOL since Sept 2023) into every RHEL/Rocky/Alma 8 install and lit up BDBA on every scan.
- **Fix** — Delete the `build-manylinux` job from `.github/workflows/build-binaries.yml`, drop `build-manylinux` from `publish-pypi.needs` and `release.needs`, remove the third `cp artifacts/coverity-metrics-linux-glibc2.28-v*/…` staging line, and update every doc reference (`INSTALL.md`, `packaging/README.md`, `release.ps1`, `RELEASE_PROCESS.md`) so the coverage table lists two Linux binaries plus a `pip install` fallback for older glibc. The two remaining standalone Linux binaries — `coverity-metrics-linux-<version>` (glibc ≥ 2.38, Ubuntu 26.04) and `coverity-metrics-linux-glibc2.35-<version>` (glibc ≥ 2.35, Ubuntu 22.04) — still ship with source-built OpenSSL 3.5.7 + zlib 1.3.2, unchanged. If the CPython 3.14 baseline in manylinux_2_28 ever moves to `libssl.so.3`, the third variant can come back with the existing job template.
- **How to recover** — Enterprise Linux hosts on glibc 2.28 – 2.34 (RHEL/Rocky/Alma 8 → 2.28, RHEL/Rocky/Alma 9 → 2.34, Amazon Linux 2023 → 2.34) should install `coverity-metrics` from PyPI instead of the release page. The wheel works on any Python 3.10+ regardless of host glibc:

  ```bash
  python3 -m venv .venv
  source .venv/bin/activate         # or: pipx install coverity-metrics
  pip install --upgrade pip
  pip install coverity-metrics
  coverity-dashboard --help
  ```

  See the new "Into a Python virtual environment" section in [INSTALL.md](INSTALL.md) for the full recipe including Windows / pipx variants. No behavioural difference from the standalone binary — same CLI, same config, same output — just no bundled CPython on the target machine.

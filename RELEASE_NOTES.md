...
- **Root cause** — Two independent problems in the same `build-manylinux` job:
  1. **Perl module deps.** OpenSSL 3.5's `./Configure` does `use IPC::Cmd qw/can_run/;` in `util/perl/OpenSSL/config.pm`, and `Makefile.in` line 37 does `use Time::Piece;`. The `quay.io/pypa/manylinux_2_28_x86_64` image ships a minimal Perl 5.26 install (enough to bootstrap Python builds) but *without* those modules — AlmaLinux 8 packages them separately from the core Perl RPM, and manylinux's slimmed base image doesn't pull them in. Configure / Make abort before compile ever starts.
  2. **SONAME mismatch on OpenSSL.** After Perl was fixed, the source-built OpenSSL 3.5.7 landed in `$HOME/openssl-new/lib64/libssl.so.3` but the manylinux CPython 3.14's `_ssl.so` is linked against `libssl.so.1.1` (OpenSSL 1.1.1k, AlmaLinux 8's system version). `LD_LIBRARY_PATH` cannot bridge across a SONAME major version bump — `ld.so` only ever considers a candidate whose SONAME matches the DT_NEEDED entry, so our 3.5.7 was invisible to `_ssl.so`. Python stayed on `OpenSSL 1.1.1k FIPS 25 Mar 2021`, tripped the `assert '3.5.7' in v` guard, and the step exited 1. This is a structural mismatch, not a workflow bug: the manylinux_2_28 image builds its CPython against the AlmaLinux 8 system OpenSSL 1.1 line, and no amount of source-building on our side can retarget an already-compiled `_ssl.so` at a different SONAME.
- **Fix** — Two coordinated changes in `.github/workflows/build-binaries.yml`:
  1. Prepend `dnf install -y perl-core` to the `build-manylinux` OpenSSL prep. `perl-core` is the AlmaLinux 8 metapackage that pulls the full standard Perl distribution — `IPC::Cmd`, `Pod::Html`, `Time::Piece`, `Digest::SHA`, `File::Compare`, `File::Path`, etc. — in one shot, so OpenSSL patchlevel bumps that touch different Perl modules won't reopen this failure. Adds ~30 MB inside the container; no runtime footprint.
  2. **Drop** the OpenSSL 3.5.7 + zlib 1.3.2 source-build steps from `build-manylinux` entirely, and replace them with an informational `Report bundled OpenSSL / zlib` step that just logs what the manylinux CPython was built against. Because of the SONAME mismatch, the source-build path physically cannot influence what OpenSSL ends up in this variant's produced binary. Building 3.5.7 anyway would add ~10 min of CI to the manylinux job and BDBA would still flag the manylinux-image OpenSSL 1.1.1k in the produced `_ssl.so`, so the freshness effort was a net negative for this variant. **This is a deliberate compatibility-over-freshness tradeoff for the `coverity-metrics-linux-glibc2.28-<version>` variant only.** The Ubuntu 26.04 / 22.04 matrix builds still source-build OpenSSL 3.5.7 + zlib 1.3.2 unchanged, because their CPythons dynamically link against `libssl.so.3` and the LD_LIBRARY_PATH override works there.
- **How to recover** — Upgrade to 1.1.10. It's the first release since the `linux-glibc2.28` variant was designed (1.1.8) where the binary actually ships. Filename / path conventions are unchanged; the coverage matrix in [INSTALL.md](INSTALL.md) and [packaging/README.md](packaging/README.md) is already correct for these artefacts. **Note on the `-glibc2.28-` variant's OpenSSL**: this specific binary bundles the OpenSSL 1.1.x that manylinux_2_28's CPython 3.14 was built against (matching AlmaLinux 8's system version). Users who need modern OpenSSL on RHEL/Rocky/Alma 8 hosts should install via `pip install coverity-metrics` from PyPI instead of taking the standalone binary, or use one of the newer-glibc binaries if their host supports it.

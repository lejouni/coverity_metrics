...
- **Fix** — Prepend `dnf install -y perl-core` to the `Build newer OpenSSL for bundling` step in `.github/workflows/build-binaries.yml`. `perl-core` is the AlmaLinux 8 metapackage that pulls the full standard Perl distribution — `IPC::Cmd`, `Pod::Html`, `Time::Piece`, `Digest::SHA`, `File::Compare`, `File::Path`, etc. — in one shot, so OpenSSL patchlevel bumps that touch different Perl modules won't reopen this failure. A first attempt (also visible in the 1.1.10 preflight history) tried a targeted `dnf install -y perl-IPC-Cmd perl-Pod-Html perl-Digest-SHA`; that got `Configure` running but the next attempt then died at `Makefile.in` line 37 with `Can't locate Time/Piece.pm`, confirming the whack-a-mole worry and pushing us to the metapackage-in-one-shot approach. `perl-core` adds ~30 MB inside the build container — no impact on the produced binary. The Ubuntu 26.04 / 22.04 matrix builds and the Windows build are unaffected: they use `apt`-installed Perl on the runner, which already carries the full standard distribution.

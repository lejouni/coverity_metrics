...
**Two coordinated release-flow gates: `release.ps1` now runs a preflight `workflow_dispatch` build and refuses to create the tag if any binary fails, and `publish-pypi` now declares `needs: [build]` so PyPI is never published when a binary build is red. Together these guarantee tags, PyPI wheels, and the GitHub Release page stay in lockstep — no more half-released 1.1.8/1.1.9-style outcomes. The 1.1.8–1.1.10 attempts to ship a third `-glibc2.28-` Linux binary from a `manylinux_2_28` container are reverted; RHEL/Rocky/Alma 8+9 and Amazon Linux 2023 users install via `pip install coverity-metrics` from PyPI instead. Docs: `INSTALL.md` now has an explicit `python -m venv` install recipe and a full air-gapped install walkthrough (wheelhouse build → transfer → `--no-index` install) for hosts with no PyPI access. No source, CLI, config, or dependency changes to the package itself.**
...
- **Root cause** — Two independent gaps:
  ...
  2. In `release.ps1`, `git tag` + `git push` ran immediately after the version-bump commit was pushed. The tag existed on origin before the release workflow finished, so a failed binary build couldn't roll the tag back.
- **Fix** — Coordinated changes on both sides:
  - **Workflow gate**: `publish-pypi` now declares `needs: [build]`. If any of the Windows / Ubuntu 26.04 linux / Ubuntu 22.04 linux-glibc2.35 matrix legs fails, PyPI publish is skipped, and the `release` job (which now `needs: [build, publish-pypi]`) is skipped too. Nothing on PyPI or the GitHub Release page ships partial.

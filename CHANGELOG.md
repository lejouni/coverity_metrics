...
### Fixed
- **1.1.9 release page again missing every Linux binary — this time because OpenSSL 3.5's `Configure` script can't run inside the `manylinux_2_28_x86_64` base image.**
  - Root cause: OpenSSL 3.5's `./Configure` does `use IPC::Cmd qw/can_run/;` in `util/perl/OpenSSL/config.pm`. The `quay.io/pypa/manylinux_2_28_x86_64` image ships a minimal Perl (5.26) with the core distribution but *without* `perl-IPC-Cmd`. Configure aborts before `make` ever runs: `Can't locate IPC/Cmd.pm in @INC (…) at util/perl/OpenSSL/config.pm line 19`, and the `Build newer OpenSSL for bundling` step exits 2. The `release` job's `needs: [build, build-manylinux, publish-pypi]` never fires, so — same failure mode as 1.1.8 — none of the three Linux binaries make it onto the release page. `publish-pypi` is independent, so the PyPI wheel for 1.1.9 published normally. This is the second install-time regression in the container job (see also 1.1.9's `pipefail`/SIGPIPE fix) and both trace back to "manylinux_2_28 is not a full CentOS 8 userland — assume nothing beyond the pre-built CPython 3.14".
  - Fix: prepend `dnf install -y perl-core` to the `Build newer OpenSSL for bundling` step in `.github/workflows/build-binaries.yml`. `perl-core` is the AlmaLinux 8 metapackage that pulls the full standard Perl distribution — `IPC::Cmd`, `Pod::Html`, `Time::Piece`, `Digest::SHA`, `File::Compare`, `File::Path`, etc. — in one shot. A first pass tried `dnf install -y perl-IPC-Cmd perl-Pod-Html perl-Digest-SHA`; that fixed `Configure` but the next preflight run then died at `Makefile.in` line 37 with `Can't locate Time/Piece.pm`, confirming the whack-a-mole worry and pushing us to the metapackage-in-one-shot approach. Downside: `perl-core` is ~30 MB of extra install; runs entirely inside the container so no runtime footprint.
  - Net effect for users: 1.1.10 is the first release since the `linux-glibc2.28` binary was designed (1.1.8) where the artifact actually ships. The Ubuntu 26.04 / 22.04 matrix builds and the Windows build are unaffected — they use `apt`-installed Perl on the runner, which already carries the full standard distribution in `libperl5.**`. No source, CLI, config, or dependency changes; `pip install coverity-metrics` on any Python 3.10+ host is unaffected.

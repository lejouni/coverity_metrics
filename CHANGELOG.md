...
### Changed
- **Release flow now refuses to tag, publish to PyPI, or attach anything to the GitHub Release page until every binary build succeeds.** The prior "tag → workflow → build + publish" order let PyPI ship 1.1.8 and 1.1.9 while binary builds failed on those same tags, so the release page ended up half-empty and the PyPI version couldn't be un-published. Two coordinated fixes:
  - **Workflow-side gate** (`.github/workflows/build-binaries.yml`): the `publish-pypi` job now declares `needs: [build]`. If any Windows / Ubuntu 26.04 / Ubuntu 22.04 build matrix leg fails, PyPI publish is skipped, and the `release` job (which requires `publish-pypi`) is skipped too. This blocks half-releases even if a tag is somehow pushed manually.
  - **Client-side gate** (`release.ps1`): after pushing the version-bump commit and before creating the tag, the script now triggers a `workflow_dispatch` of `build-binaries.yml` on the just-pushed SHA and waits (`gh run watch --exit-status`) for every build job to finish green. Only then are `git tag` and `git push $Remote $tag` allowed to run. If preflight fails, the script aborts with a clear error, nothing is tagged, and PyPI / GitHub Release stay untouched. A new `-SkipPreflightCI` switch falls back to the legacy "tag and hope" flow for exceptional cases; `-DryRun` narrates the preflight without dispatching.

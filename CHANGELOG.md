...
### Added
- **`INSTALL.md` now documents a plain `python -m venv` install recipe alongside `pipx`.** The previous "Installing without admin or root rights" section covered `pip install --user` and `pipx`, but had no explicit walkthrough for users who prefer managing their own virtual environment (e.g. one venv per project, or wanting to see exactly what pipx does under the hood). New subsection between "From PyPI" and "Installing without admin or root rights" gives copy-paste-ready snippets for Linux/macOS bash/zsh, Windows PowerShell, and Windows cmd.exe — venv creation, activation, install, CLI verification, deactivate — plus the standard pins/notes (Python 3.10+ required, delete `.venv/` to wipe the install, `pip install coverity-metrics==X.Y.Z` for a specific version). Points at `pipx` for readers who want the same isolation without hand-managing venvs.
- **`INSTALL.md` now covers air-gapped install (no internet on the target host).** Motivated by the removal of the `coverity-metrics-linux-glibc2.28-<version>` binary this release — RHEL/Rocky/Alma 8/9 users can no longer grab a self-contained binary, so they need a doc-supported way to `pip install` without PyPI access. New "Air-gapped install" section in [INSTALL.md](INSTALL.md) walks through the standard three-step wheelhouse workflow: (1) on an internet-connected machine, `pip download --dest wheelhouse --only-binary=:all: coverity-metrics pip setuptools wheel`, optionally pinned to the target's platform / Python via `--platform / --python-version / --abi` (table of common tags: `manylinux2014_x86_64`, `manylinux_2_28_x86_64`, `win_amd64`, `macosx_11_0_arm64`, `macosx_10_9_x86_64`); (2) `tar czf` (or `Compress-Archive` on Windows) and transfer; (3) on the target, `pip install --no-index --find-links wheelhouse coverity-metrics` inside a venv. Includes a troubleshooting block for the three common failure modes (missing platform-specific wheel during download, wheelhouse ABI mismatch during install, sdist accidentally slipping in when `--only-binary=:all:` is dropped) and notes on `pipx` compatibility + upgrade / storage-footprint expectations. No workflow / CI change: the recipe uses stock `pip` and standard PyPI, so `pip install coverity-metrics` on Python 3.11+ is the single supported install surface.

### Changed
- **Release flow now refuses to tag, publish to PyPI, or attach anything to the GitHub Release page until every binary build succeeds — and the tag-triggered path no longer rebuilds the binaries.** Three coordinated changes:
  - **Workflow-side gate** (`.github/workflows/build-binaries.yml`): the `build` job now runs *only* on `workflow_dispatch` (via `if: github.event_name == 'workflow_dispatch'`), so pushing a tag doesn't kick off a duplicate matrix build. A new `find-preflight-run` job runs on tag push, calls `gh run list --workflow=build-binaries.yml --status=success --json databaseId,headSha` to locate the preflight run that produced artifacts for the tag's commit, and hands its `run_id` to the `release` job. `release` now uses `actions/download-artifact@v5` with `run-id:` + `github-token:` for a cross-run download instead of re-building everything. `publish-pypi` still runs but only builds the sdist/wheel (fast) — it no longer needs the matrix `build`. Net effect: tag push time drops from ~15 min (build + publish + release) to ~2 min (find + publish + release). If someone pushes a tag against a commit that never had a green preflight, `find-preflight-run` fails loudly with a message pointing at `release.ps1`. Added `permissions: actions: read` at workflow scope so the `gh run list` + cross-run download have the token scopes they need (`contents: write` was already there for GitHub Release attachment).
  - **Client-side gate** (`release.ps1`): after pushing the version-bump commit and before creating the tag, the script triggers a `workflow_dispatch` of `build-binaries.yml` on the just-pushed SHA and waits (`gh run watch --exit-status`) for every build job to finish green. Only then are `git tag` and `git push $Remote $tag` allowed to run. If preflight fails, the script aborts with a clear error, nothing is tagged, and PyPI / GitHub Release stay untouched. A `-SkipPreflightCI` switch falls back to the legacy "tag and hope" flow for exceptional cases; `-DryRun` narrates the preflight without dispatching.
  - **Half-release protection**: even if a tag is pushed by hand (skipping `release.ps1` entirely), `find-preflight-run` still refuses to hand a run-id to `publish-pypi` / `release` unless a green preflight run exists for that exact commit SHA. There is no code path where PyPI publishes without matching binaries.
  - Requires the [GitHub CLI](https://cli.github.com/) (`gh`) installed and authenticated (`gh auth login`) — the script emits an actionable error if `gh` isn't on PATH.

### Removed
- **The `coverity-metrics-linux-glibc2.28-<version>` binary is gone.** The `build-manylinux` container job that produced it (`quay.io/pypa/manylinux_2_28_x86_64`) is removed from `.github/workflows/build-binaries.yml`, `publish-pypi.needs` drops back to `[build]`, `release.needs` drops back to `[build, publish-pypi]`, and the third `cp artifacts/coverity-metrics-linux-glibc2.28-v*/…` staging line is deleted. The 1.1.8/1.1.9/1.1.10 attempts to ship this variant collectively hit five distinct failures inside that job (`ldd | head` SIGPIPE, `Can't locate IPC/Cmd.pm`, `Can't locate Time/Piece.pm`, `LD_LIBRARY_PATH` failing to override the `libssl.so.1.1` SONAME `_ssl.so` was linked against, and finally the OpenSSL assertion still tripping after the fresh source-build was dropped) — see the 1.1.8 / 1.1.9 changelog entries and the removed CI logs for the trail. Root cause is structural, not fixable in-workflow: manylinux_2_28's pre-built CPython 3.14 is linked against `libssl.so.1.1` from AlmaLinux 8's system OpenSSL 1.1.1k, and no combination of `LD_LIBRARY_PATH`, `perl-core`, or venv-based bootstrap can retarget an already-compiled `_ssl.so` at a different SONAME. Shipping the binary anyway would have baked EOL OpenSSL 1.1.1k into every RHEL/Rocky/Alma 8 install and lit up BDBA every scan. **Users on glibc 2.28 – 2.34 (RHEL/Rocky/Alma 8+9, Amazon Linux 2023, older enterprise hosts) now install via `pip install coverity-metrics` from PyPI** — the wheel works on any Python 3.10+ regardless of host glibc. `INSTALL.md` and `packaging/README.md` were updated to document the `pip install` fallback in the coverage table, and the new "Into a Python virtual environment" section in `INSTALL.md` gives the exact recipe. The two remaining standalone Linux binaries (`coverity-metrics-linux-<version>` on glibc ≥ 2.38 and `coverity-metrics-linux-glibc2.35-<version>` on glibc ≥ 2.35) still ship with source-built OpenSSL 3.5.7 + zlib 1.3.2, unchanged. If the CPython 3.14 baseline in manylinux_2_28 ever moves to `libssl.so.3`, the third variant can come back with the existing job template.
...
- **Second Linux binary for older glibc hosts (`coverity-metrics-linux-glibc2.35-<version>`).**
  - Trigger: on hosts running RHEL/Rocky/Alma 9-era or Ubuntu 22.04-era userland, the primary `coverity-metrics-linux-<version>` binary (built on Ubuntu 26.04) aborts at launch with `[PYI-...:ERROR] Failed to load Python shared library '.../libpython3.14.so.1.0': /lib64/libm.so.6: version 'GLIBC_2.38' not found`. That's PyInstaller loading the bundled CPython 3.14 shared object, which was linked against the Ubuntu 26.04 runner's glibc 2.38. Any host below 2.38 (Ubuntu 22.04 → 2.35, Debian 12 → 2.36, RHEL/Rocky 9 → 2.34, RHEL 8 → 2.28) refuses to load it.
  - Fix: added a second matrix entry to `.github/workflows/build-binaries.yml` that runs on `ubuntu-22.04` and publishes the artifact as `coverity-metrics-linux-glibc2.35-<version>`. The runner's glibc 2.35 caps the symbol requirements on the bundled `libpython3.14.so.1.0`, so the resulting binary runs on Ubuntu 22.04+, Debian 12+, Fedora 36+ and any newer host. The OpenSSL 3.5.7 / zlib 1.3.2 source-build steps are still applied (their `if:` guards were widened from `matrix.artifact_os == 'linux'` to `runner.os == 'Linux'`), so BDBA findings track the modern binary. The original `coverity-metrics-linux-<version>` still ships unchanged for hosts on the newer glibc line.
  - Release-page globbing was tightened: `coverity-metrics-linux-v*` now matches only the modern-glibc artifact folder (anchoring on the `-v` prefix of the tag), and a second `coverity-metrics-linux-glibc2.35-v*` copy line stages the legacy binary next to it. Both binaries are `chmod +x`-ed and attached to the release with self-documenting filenames.
  - Documentation updated in [INSTALL.md](INSTALL.md) (new "Which Linux binary?" callout with an `ldd --version` check and the coverage matrix) and [packaging/README.md](packaging/README.md) (revised download list and "Notes" block; dropped the stale "built on `ubuntu-latest` (glibc 2.39)" claim that predated the current pin to Ubuntu 26.04).
  - Neither binary covers RHEL/Rocky/Alma 9 (glibc 2.34) or RHEL 8 (2.28) — those hosts still need `pip install coverity-metrics` from PyPI. A future manylinux_2_28 container build is tracked as a follow-up.
...
- **`MULTI_INSTANCE_GUIDE.md` rewritten to match the 1.1.x code — the previous copy described the 1.0.x design.**
  - **Output layout.** Guide showed a flat `output/dashboard_Production.html` / `output/dashboard_Production_MyApp.html` layout that hasn't shipped since 1.0.x. Current layout is one folder per instance (`.replace(' ', '_')`-sanitised) with `dashboard.html` at the folder root and `dashboard_<Project>.html` per project — see `coverity_metrics/cli/dashboard.py` around L1746 / L1889 / L2307. The "Generated Dashboards" tree, the per-instance and per-project bullet lists, and the "Migration from Single Instance" step-5 note were all updated to reflect the folder layout, including the space-to-underscore folder rule.
  - **Python API import path.** Every `from multi_instance_metrics import MultiInstanceMetrics` in the guide (three code blocks — connection pooling, programmatic access, scheduled reporting) rewritten to `from coverity_metrics import MultiInstanceMetrics`, matching the actual re-export in `coverity_metrics/__init__.py`. The bare-module import path never worked from an installed package; it only worked in-repo when `PYTHONPATH` happened to include the `coverity_metrics/` directory.
  - **`${VAR}` interpolation section deleted.** The old "Environment Variables (Alternative)" recipe told users to put `"password": "${COVERITY_PROD_PASSWORD}"` in `config.json` and export the variable. Nothing in `InstanceConfig.__init__` or `_load_config` calls `os.path.expandvars` / does `${…}` substitution — the string was passed through verbatim to pg8000, so the recipe never actually worked. Replaced with an honest note that `config.json` reads passwords verbatim, plus a pointer at the env-var mode that does exist: `coverity-export --env` reading `COVERITY_DB_HOST / COVERITY_DB_NAME / COVERITY_DB_USER / COVERITY_DB_PASSWORD` (and optional `COVERITY_DB_PORT`, `COVERITY_INSTANCE_NAME`) for a single instance. Cross-references `EXPORT_QUICKSTART.md` and `MULTI_ZIP_GUIDE.md` for the ZIP-based downstream workflow.
  - **`default_instance` removed from the example JSON.** The guide advertised a `default_instance: "Production"` field. `grep default_instance **/*.py` finds no consumer — it was never wired. Dropped from the example.
  - **"Migration from Single Instance" section rewritten.** Old copy told users to `cp db_config.ini db_config.ini.backup`; no `db_config.ini` file has existed in the repo for many releases. Rewrote as "Growing a single-instance setup into a cluster" — auto-detection is the whole migration story, `config.json` is the authoritative single-instance shape too, and `--single-instance-mode` on the CLI overrides auto-detection.
  - **Troubleshooting connectivity check corrected.** Old copy said `python multi_instance_metrics.py`; the file lives inside the package with no `__main__` block, so that command errored. Replaced with `coverity-dashboard --instance Production --no-browser` (or the `python -m coverity_metrics dashboard …` equivalent) plus a `psql` line for the raw connectivity test.
  - **API reference link corrected.** Was `[multi_instance_metrics.py](multi_instance_metrics.py)`; now `[coverity_metrics/multi_instance_metrics.py](coverity_metrics/multi_instance_metrics.py)` with a one-liner reminding readers the public surface is re-exported from the package root.
  - **New content that was missing.** Called out `--instance-only` (skip per-project dashboards); linked out to `MULTI_ZIP_GUIDE.md` for the ZIP-mode aggregated-view opt-in (`--aggregated-view` or `zip_files_config.aggregated_view.enabled`); noted the 1.1.6 duplicate-instance-name disambiguation and space-to-underscore folder rule inline.
- **`config.json.example` — dropped the unused `default_instance` field.** Same reason as the guide: no code path reads it. Removing it prevents new users from wiring config around a knob that does nothing.

### Fixed
- **`MultiInstanceMetrics._load_config()` fallback path was wrong: `config.example.json` doesn't exist, the shipped filename is `config.json.example`.**
  - When `config.json` was missing, `_load_config()` in `coverity_metrics/multi_instance_metrics.py` printed a warning and pointed `self.config_file` at `config.example.json` before the `open(...)` call. The repo ships `config.json.example` (with the `.example` suffix at the end, matching how every other tool in this space names it). The fallback therefore always raised `FileNotFoundError` from the subsequent `open(...)`, so the warning "using config.example.json" was never actionable.
  - Fix: rename the fallback string to `config.json.example` and refresh the surrounding comment to say what the code cannot show on its own (i.e. that this is the file the repo actually ships). Behaviour with a caller-supplied `config.json` present is unchanged.

## [1.1.6] - 2026-09-02

### Added
- **`coverity-dashboard --aggregated-view` — opt in to the cross-instance aggregated dashboard in ZIP mode without needing `config.json`.**
  - Historically, `dashboard_aggregated.html` was only generated when `--config` pointed at a JSON file whose `zip_files_config.aggregated_view.enabled` was `true`. Users who ran ZIP-mode workflows (no database, no config) had no way to opt in from the CLI. `config.json` also became a de-facto requirement for behaviour that had nothing to do with database connection details.
  - The new `--aggregated-view` flag (default `False`) enables the aggregated dashboard directly. The config-file switch still works — either source alone is enough. There is no `--no-aggregated-view`; to force off, omit the flag and/or set `enabled: false` in the config.
  ...
  - Scoped to ZIP mode only. Database mode continues to read `aggregated_view.enabled` from `config.json` (the config is already required there for connection details).

### Fixed
- **`coverity-dashboard --zip-file archive/*.zip` failed on Windows with `[ERROR] ZIP file not found: archive/*.zip` because PowerShell and `cmd` do not expand globs.**
  - Root cause: on Windows, unlike POSIX shells, the shell hands `*` / `?` / `[…]` to the tool verbatim. The old code fed the literal pattern straight into `os.path.exists(...)` and exited. The multi-ZIP workflow documented across the README, MULTI_ZIP_GUIDE.md and EXPORT_QUICKSTART.md was effectively broken on Windows.
  - Fix: expand `--zip-file` entries with `glob.glob(pattern)` immediately after argv parsing. Literals (no glob metacharacters) pass through unchanged. Matches are sorted for deterministic ordering; the surrounding argv order is preserved. Patterns that match nothing are left in place so the existing "ZIP file not found" line reports them with the offending pattern rather than silently dropping them. Each expanded glob prints a `[OK] Glob '<pattern>' matched N ZIP(s)` line for transparency. Works identically on Linux (where the shell would have expanded it) and on Windows.
- **Multiple `--zip-file` inputs that carried the same internal instance name silently collapsed into a single row in the aggregated dashboard.**
  - Root cause: the multi-ZIP loading loop in `coverity_metrics/cli/dashboard.py` did `zip_loaders[instance_name] = loader`, using the first available instance name from each ZIP's metadata as the dict key. When two or more ZIPs shared that name (for example, every ZIP labelled `Production` even though the exports came from different Coverity servers), later entries overwrote earlier ones. Only one row appeared in the aggregated defect / user / DB tables, per-instance folders collided, and the losers' totals were silently dropped from every summed metric.
  - Fix: extract the loading loop into `_build_zip_loader_map(zip_files, loader_cls=ZipDataLoader, logger=None)`. It runs a two-pass build — first collects `(zip_file, raw_name, loader)` tuples, then counts duplicates and assigns a `display_name` per entry. Unique names pass through unchanged; duplicates get the ZIP filename's stem appended in parentheses (e.g. `Production (prod_us)`, `Production (prod_eu)`). One `[INFO]` line per collision reports the raw name and the ZIP basenames involved. The disambiguated label is exposed as `loader.display_name` and flows through the dict key + `all_instances` list into per-instance output folders, dashboard titles, and colors. `loader.instance_name` deliberately stays at the raw ZIP-metadata value because `ZipDataLoader` doubles it as the folder prefix inside the archive (`f"{instance_name}/{metric_name}.json"`) — overwriting it with the display name would make every downstream metric read empty. Argv order of ZIPs is preserved so the color-palette rotation stays deterministic across runs.
...
- **Links from `dashboard_aggregated.html` to per-instance dashboards were broken whenever an instance name contained a space.**
  - Root cause: the aggregated template emitted hrefs as `{{ inst.name }}/dashboard.html` and `{{ inst.instance_name }}/dashboard.html` — the raw label with spaces preserved. Per-instance folders on disk are created with `.replace(' ', '_')`, so the browser looked for `Production (NAM)/dashboard.html` while the actual folder was `Production_(NAM)/dashboard.html`. Latent for years for DB-mode users with space-containing instance names (e.g. `"Production Coverity"`), and freshly exposed in 1.1.6 by the duplicate-instance-name disambiguation, which always injects spaces into the display name (`Production (prod_us)`).
  - Fix: apply `.replace(' ', '_')` to every `inst.name` / `inst.instance_name` used as a URL segment inside `coverity_metrics/templates/dashboard_aggregated.html` (Jump-to-Instance dropdown, Quick Instance Cards, per-instance table row `onclick` + hyperlink). Visible labels (`{{ inst.name }}` / `{{ inst.instance_name }}` in the surrounding text and `<option>` display strings) keep their spaces, so users still see the pretty `Production (prod_us)` name. Same convention the per-instance `dashboard.html` template already uses for project links at [dashboard.html#L80](coverity_metrics/templates/dashboard.html#L80).
- **Per-instance ZIP-mode dashboards showed a "🌐 Back to All Instances" button linking to `../dashboard_aggregated.html` even when the aggregated dashboard hadn't been generated (i.e. `--aggregated-view` was omitted and no config file enabled it).**
  - Root cause: five `generate_html_dashboard(...)` call sites in the ZIP branches of `coverity_metrics/cli/dashboard.py` didn't forward `has_aggregated_dashboard` and fell back to the argument's default of `True`. The template guard `{% if current_instance and not current_project and has_aggregated_dashboard %}` therefore still rendered the button, and clicking it produced a broken link to a file that was never created. DB-mode already forwarded `aggregated_view_enabled` at every site — this was purely a ZIP-mode omission.
  - Fix: pass `has_aggregated_dashboard=zip_aggregated_view_enabled` to every `generate_html_dashboard(...)` call in the ZIP branches (multi-ZIP instance-level + per-project, single-ZIP instance-level + sequential per-project + parallel `_render` worker). Per-project dashboards never render the button (the template gate also requires `not current_project`), so the extra kwarg on them is belt-and-braces — but it means the plumbing stays consistent and any future template that surfaces the same button at project level will Just Work.
  - Same fix applied to the "🌐 All Instances" breadcrumb link at the top of every ZIP-mode per-instance and per-project dashboard — it was rendered whenever `current_instance` was truthy, without checking `has_aggregated_dashboard`. Wrapped the link segment (`<a href="../dashboard_aggregated.html">🌐 All Instances</a>` + trailing `›` separator) in `{% if has_aggregated_dashboard %}` inside `coverity_metrics/templates/dashboard.html`. The rest of the breadcrumb (`{{ current_instance }} › {{ current_project }}`) still renders, so users see where they are — just without the misleading link when there's nowhere to go back to.
- **`coverity-dashboard` crashed with `TypeError: '>' not supported between instances of 'NoneType' and 'int'` when rendering the Trends / Scan activity / Snapshot performance tables against a ZIP whose per-period `new_defects` (or `total_new_defects`) column was `None`**
  - Root cause: three `<td class="metric-number {% if row.new_defects > 10 %}metric-increase{% endif %}">` guards in `coverity_metrics/templates/dashboard.html` compared the raw column value against `10` without a null coalesce. The 1.1.5 `_sanitize_for_template` NaN→`None` rewrite made this reachable on real data — any `NULL`-bearing aggregate row that survived sanitization now reached the Jinja comparison as literal `None`, and `None > 10` raises `TypeError` in Jinja's Python-mode expression evaluator, aborting the whole `template.render(...)` call before any HTML was written. The nearby `{{ row.new_defects or 0 }}` interpolation already handled the display side; only the CSS-class conditional was missing the same guard.
  - Fix: wrap each comparison in `(row.<col> or 0) > 10` so `None` collapses to `0` before the comparison. Three sites patched, all in `dashboard.html`: `total_new_defects` on the scan-activity table (~line 957), `new_defects` on the defect-trends table (~line 1050), and `new_defects` on the snapshot-performance table (~line 1426). No template-model change, no CLI-flag change, no cache-key change — existing dashboards regenerate cleanly on affected datasets.
...
- **New `coverity-delta` CLI: multi-snapshot trend comparison report from a folder of `coverity-export` ZIPs**
  - Consumes every `*.zip` in a folder produced by iterated `coverity-export` runs, orders them chronologically (oldest → newest), and emits a machine-readable `delta.json` (schema v1, trend shape) plus a self-contained `dashboard_delta.html` with inline SVG line charts and per-project sparklines. Runs entirely offline — no database access. Ships as a fourth console script alongside `coverity-dashboard` / `coverity-metrics` / `coverity-export`, and as the `delta` subcommand on `python -m coverity_metrics` and the standalone binary.
  - **Single input mode**: `coverity-delta --archive-dir archive/` where `archive/` contains two or more `coverity-export` ZIPs. `--previous` / `--current` are gone — the folder is the sole input. Minimum 2 ZIPs; no upper bound. Snapshot labels auto-derive to `YYYY-QN` from each ZIP's `export_date`; `--labels a,b,c,...` overrides them positionally in chronological order.
  - **Chronological ordering**: primary key is `export_timestamp` inside each ZIP's `metadata.json` (robust — survives copying / re-archiving); falls back to filename `YYYYMMDD_HHMMSS` when metadata is missing, then to filesystem mtime as a last resort.
  - **Trend metric families**: (1) **projects** — per-snapshot count series + first→last added / dropped / retained set diff (from `total_defects_by_project.json`); (2) **active users** — per-stat time series for `total_licensed_users`, `users_with_login`, `active_users`, and the two percentage columns, plus a first→last set diff on `top_users_by_fixes.json`; (3) **scan activity** — per-snapshot window totals for snapshots / files analyzed / defects introduced / defects eliminated (from `scan_activity_trend.json`); when `--days` differs across the archive a `normalized_per_day` series is emitted alongside raw totals so the chart stays meaningful; (4) **stream activity** — per-snapshot active-stream count series + first→last "newly active" / "went dark" streams (from `snapshot_performance.json` — the underlying metric is a top-N sample so per-stream count deltas remain deliberately out of scope, only presence flips are surfaced); (5) **defects by project** — one row per project (union across the whole window) with a sparkline of `active_defects` values across every snapshot (nulls break the line where the project was absent), first / last active values, first→last Δ + %Δ, rank series + first / last rank + rank movement (▲ climbed, ▼ dropped, `new` / `dropped` for asymmetric appearance). Ranking is recomputed per snapshot via dense-rank on `active_defects`.
  - **Recommended workflow** — see the "Trend Comparison Workflow" section in `README.md` and the appended section in `EXPORT_QUICKSTART.md`: at the end of every quarter run `coverity-export --days 90 --anonymize --mapping-file archive/shared-mapping.json --output archive/`, letting the ZIP land in the shared archive folder. After N quarters, run `coverity-delta --archive-dir archive/` — the trend report picks up every ZIP in the folder in chronological order. The shared `--mapping-file` keeps `project_001` pointing at the same real project across every quarter; `--days` must match across every ZIP for windowed metrics to stay comparable. Both are enforced by the CLI's validation gate across the whole archive.
  - **Guardrails, all hard-fails, all applied across the whole archive** (not just a single pair): (a) instance-list mismatch between any two ZIPs → refuse, showing the offending pair; (b) `--days` mismatch between any two ZIPs → refuse, with `--allow-window-mismatch` escape hatch documented as testing / advanced use only (report is emitted anyway with a `window_mismatch` entry in `delta.json.warnings`, ⚠ markers on affected sections in the HTML, and a `normalized_per_day` series alongside raw totals); (c) mismatched anonymization mappings (different `<zip>.mapping.json` fingerprints across ZIPs) → refuse with no override — fix by rerunning every export with the same `--mapping-file`. Missing mapping on a subset is downgraded to a `mapping_missing_some` warning so users trying delta without anonymization still get a report. Empty folder / single-ZIP folder / missing folder → clear error messages with fix hints, not a Python traceback.
  - **HTML report** is self-contained (inline CSS, no external CDN dependencies, no JS charting libs): header with the full snapshot chain (`2026-Q1 → 2026-Q2 → 2026-Q3 → ...`), warnings ribbon, and per-instance sections rendering hand-rolled inline SVG line charts for every metric family. All three top sections (Projects, Active users, Stream activity) follow the same rhythm: tile-grid summary → line chart → detail lists (matches the muscle-memory of the main `coverity-dashboard`). Per-project defect table uses an inline SVG sparkline column with a `data-sort-value` attribute (the first→last active-defects Δ) so the repo's sortable-header autoloader keeps sorting the column even when the cell is a chart. Every chart marker — line-chart circles *and* sparkline circles — wraps a native SVG `<title>` element so hovering any data point reveals `{label}: {value}` (e.g. `Q3/26: 4`) without a single line of JavaScript. Deliberately zero JS deps beyond the sortable-headers snippet copied from `templates/dashboard.html`.
  - **Timeline detail on added / dropped / newly active / went dark lists**: each entry in these four lists carries `{name, first_seen_index, last_seen_index, snapshot_count}` in `delta.json`, and the HTML renders a colored tag next to each name — `joined Q2/26` (blue, still present), `last seen Q3/26` (blue, was at start), or `Q1/26 → Q3/26 (came & went)` (yellow, appeared and disappeared mid-window). The last case shows the same entry in both Added and Dropped lists — deliberate, matches the compute semantic below.
  - **Set-diff semantic: "ever appeared / ever removed" across the window, not just first-vs-last endpoints**. The four lists on Projects and Stream activity use `union(all snapshots) - first` for added and `union(all snapshots) - last` for dropped. This catches projects (or streams) that were created and then deleted between the endpoints — which a plain `first - last` diff misses even though the count-series line chart makes the intermediate dip visible. Retained is still `first ∩ last`. Verified against the two-endpoints test case (project created and removed inside a single archive) — the count series showed the spike, and the tile now matches.
  - **Endpoint semantics on the Defects by project table**: `rank_first` / `rank_last` / `active_first` / `active_last` come from the *first snapshot slot* and *last snapshot slot* respectively (`None` if the project was absent at that endpoint), not from the first / last *observed* value. A late-arriving project reports `rank_first = None` and drives the HTML "new" indicator; a project removed before the end reports `rank_last = None` and drives "dropped"; a project absent at *both* endpoints (transient) drives a new "came & went" indicator in the ranking column. Without this fix, a project present only in the middle snapshots reported `rank_first = rank_last = <some observed rank>` and rank Δ = 0, hiding the fact that it had been deleted entirely.
  - Ships with 28 unit + end-to-end tests (`test_delta_metrics.py` = 18, `test_delta_cli.py` = 10) covering: `SnapshotLoader.order_key` metadata → filename → mtime fallback chain; `load_archive_dir` chronological ordering when filenames disagree with metadata; empty / single-ZIP archive rejection; each `compute_*_series` function including missing-snapshot handling (`None` slots in `active_series` / `rank_series`); **regression test for the mid-window added-and-deleted case** (`test_compute_projects_series_catches_added_and_dropped_mid_window`); **regression test for the endpoint-semantic fix on Defects by project** (`test_defects_by_project_series_endpoint_semantics_for_late_arrival_and_early_departure`); trend orchestration; and every CLI guardrail (empty dir, single-ZIP dir, mid-archive instance / days / mapping mismatch, `--allow-window-mismatch` escape hatch, `--labels` override).

## [1.1.3] - 2026-08-20

### Changed
- **Linux binary CI now builds pristine OpenSSL 3.5.7 and zlib 1.3.2 from source and bundles them via `LD_LIBRARY_PATH` so PyInstaller ships those instead of the runner's system copies**
  - Ubuntu 26.04's system OpenSSL is `libssl3 3.5.5` (Jan 2026 release, frozen at the string Canonical shipped with the LTS). BDBA against the 1.1.2 Linux binary flagged this as behind upstream — OpenSSL 3.5.7 shipped shortly after and the LTS won't bump the version string, only backport security patches within `3.5.5-*ubuntu*`. The 1.1.2 fix ("pin the runner OS to bump the system OpenSSL") therefore has a fundamental ceiling: it only moves as fast as Ubuntu's package-string cadence.
  - New workflow step builds OpenSSL 3.5.7 from the official `openssl/openssl` release tarball into `$HOME/openssl-new`, then prepends `$HOME/openssl-new/lib64:$HOME/openssl-new/lib` to `LD_LIBRARY_PATH` for the remainder of the job. PyInstaller resolves the CPython `_ssl.so` and `_hashlib.so` shared-library dependencies through `ldd`, which honors `LD_LIBRARY_PATH`, so the bundle picks up the freshly-built `libssl.so.3` / `libcrypto.so.3` instead of the runner's system ones. Stayed within the 3.5.x line so the ABI stays compatible with the `_ssl.so` that CPython 3.14 was compiled against.
  - Same approach applied to zlib: the runner's system `libz.so.1` is Debian's `1:1.3.dfsg+really1.3.1-1ubuntu3` (Debian applies non-upstream patches over 1.3.1 and marks the string `dfsg`), which some scanners flag against pristine upstream. The workflow now downloads `zlib-1.3.2.tar.gz` from `zlib.net`, builds it into `$HOME/zlib-new`, and prepends the lib dir to `LD_LIBRARY_PATH`. Bundle gets pristine upstream `libz.so.1` at ABI-compatible soname.
  - `OPENSSL_VER` / `ZLIB_VER` are declared as top-level `env` on the respective build steps so future bumps are a one-line diff — see the workflow comments for the upstream sources to check.
  - Verification steps in each build phase confirm `python -c "import ssl; print(ssl.OPENSSL_VERSION)"` returns the new string and that `ldd` on `_ssl.so` / `zlib.so` resolves to the new lib dirs before the PyInstaller invocation runs.
  - A new "Diagnostic - identify libcrypto/libz sources" step runs first on Linux to log the pre-override paths, versions, and Ubuntu package strings — kept in-place so future BDBA deltas can be root-caused against the actual bundled artifact without reproducing the runner locally.
- **Windows binary is unaffected** — Windows CPython bundles OpenSSL and zlib inside its own installer (`Python314\DLLs\libcrypto-3.dll`, `Python314\DLLs\libssl-3.dll`, `Python314\DLLs\zlib.dll`), which track CPython's own release cadence. No `LD_LIBRARY_PATH`-equivalent hack is needed on Windows because PyInstaller pulls those DLLs directly from the CPython install, not from a system-wide location.

### Fixed
- **`EXPORT_QUICKSTART.md` and `scripts/coverity-export.ps1` docs now recommend `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass` instead of `RemoteSigned`**

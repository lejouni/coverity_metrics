...
- **Release flow now refuses to tag, publish to PyPI, or attach anything to the GitHub Release page until every binary build succeeds.** The prior "tag → workflow → build + publish" order let PyPI ship 1.1.8 and 1.1.9 while `build-manylinux` failed on those same tags, so the release page ended up half-empty and the PyPI version couldn't be un-published. Two coordinated fixes:
  - **Workflow-side gate** (`.github/workflows/build-binaries.yml`): the `publish-pypi` job now declares `needs: [build, build-manylinux]`. If any Windows / Ubuntu 26.04 / Ubuntu 22.04 build matrix leg or the manylinux_2_28 container job fails, PyPI publish is skipped, and the `release` job (which already required both) is skipped too. This blocks half-releases even if a tag is somehow pushed manually.
  ...
  - Requires the [GitHub CLI](https://cli.github.com/) (`gh`) installed and authenticated (`gh auth login`) — the script emits an actionable error if `gh` isn't on PATH. No workflow_dispatch runs are billed against the private PyPI tier because they run on GitHub-hosted runners just like the tag-triggered path.

### Fixed
- **1.1.9 release page again missing every Linux binary — first because OpenSSL 3.5's `Configure` couldn't run inside the `manylinux_2_28_x86_64` base image, then because `_ssl.so`'s libssl SONAME didn't match our source-built OpenSSL.**
  - Root cause 1 (Perl module deps): OpenSSL 3.5's `./Configure` does `use IPC::Cmd qw/can_run/;` in `util/perl/OpenSSL/config.pm`; a follow-up preflight run got past that and then died at `Makefile.in` line 37 with `Can't locate Time/Piece.pm`. The `quay.io/pypa/manylinux_2_28_x86_64` image ships a minimal Perl 5.26 that carries neither. Configure / Make abort before the compile ever starts, `build-manylinux` exits 2, and the `release` job's `needs: [build, build-manylinux, publish-pypi]` never fires, so — same failure mode as 1.1.8 — none of the three Linux binaries make it onto the release page.
  - Root cause 2 (SONAME mismatch): after the Perl deps were installed, `Configure` + `make install_sw` succeeded and produced `libssl.so.3` / `libcrypto.so.3` in `$HOME/openssl-new/lib64/`. But the manylinux CPython 3.14's `_ssl.so` is linked against `libssl.so.1.1` (OpenSSL 1.1.1k, the system version on AlmaLinux 8), not `libssl.so.3`. `LD_LIBRARY_PATH` cannot bridge across a SONAME major version bump: `ld.so` only ever considers a candidate whose SONAME matches the DT_NEEDED entry, so our OpenSSL 3.5.7 was invisible to `_ssl.so` and Python kept resolving `libssl.so.1.1` from the system. `ssl.OPENSSL_VERSION` stayed on `1.1.1k FIPS 25 Mar 2021`, tripped the `assert '3.5.7' in v` guard, and the step exited 1. This is a structural mismatch, not a workflow bug: the manylinux_2_28 image simply builds its CPython against the AlmaLinux 8 system OpenSSL 1.1 line, and no amount of source-building on our side changes that.
  - Fix: two coordinated changes in `.github/workflows/build-binaries.yml`.
    - Prepend `dnf install -y perl-core` to the `build-manylinux` OpenSSL prep to fix Root cause 1. `perl-core` is the AlmaLinux 8 metapackage that pulls the full standard Perl distribution — `IPC::Cmd`, `Pod::Html`, `Time::Piece`, `Digest::SHA`, `File::Compare`, `File::Path`, etc. — in one shot, so OpenSSL patchlevel bumps that touch different Perl modules won't reopen this failure. Adds ~30 MB inside the container; no runtime footprint.
    - Then **drop** the OpenSSL 3.5.7 + zlib 1.3.2 source-build steps from `build-manylinux` entirely, and replace them with an informational `Report bundled OpenSSL / zlib` step that just logs what the manylinux CPython was built against. Rationale: because of the SONAME mismatch above, the source-build path physically cannot influence what OpenSSL ends up in this variant's produced binary — `_ssl.so` will always resolve `libssl.so.1.1` at load time. Building 3.5.7 anyway would add ~10 min of CI to the manylinux job and BDBA would still flag the manylinux-image OpenSSL 1.1.1k in the produced `_ssl.so`. This is a deliberate compatibility-over-freshness tradeoff for the `coverity-metrics-linux-glibc2.28-<version>` variant only; the Ubuntu 26.04 / 22.04 matrix builds still source-build OpenSSL 3.5.7 + zlib 1.3.2 unchanged, because their CPythons are dynamically linked against `libssl.so.3` and the LD_LIBRARY_PATH override works there.
  - Net effect for users: 1.1.10 is the first release since the `linux-glibc2.28` binary was designed (1.1.8) where the artifact actually ships. This one variant carries the manylinux_2_28 image's OpenSSL (1.1.x line on AlmaLinux 8); users who need modern OpenSSL on RHEL/Rocky/Alma 8 hosts should install via `pip install coverity-metrics` from PyPI instead, or use the `-glibc2.35-` / primary binaries on hosts where those are available. No source, CLI, config, or dependency changes; `pip install coverity-metrics` on any Python 3.10+ host is unaffected.

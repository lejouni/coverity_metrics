...
- **New `coverity-delta` CLI: quarter-over-quarter comparison report from two `coverity-export` ZIPs**
  - Consumes two ZIPs produced by `coverity-export` (a "previous" and a "current" snapshot) and emits a machine-readable `delta.json` (schema v1) plus a self-contained `dashboard_delta.html`. Runs entirely offline — no database access. Ships as a fourth console script alongside `coverity-dashboard` / `coverity-metrics` / `coverity-export`, and as the `delta` subcommand on `python -m coverity_metrics` and the standalone binary.
  - **Adoption-focused metric families in v1.1.4**: (1) projects added / dropped / retained (from `total_defects_by_project.json`); (2) active-user Δ — `total_licensed_users`, `users_with_login`, `active_users`, plus percentage columns — with an optional set diff on the top-fixers list (from `user_license_statistics.json` + `top_users_by_fixes.json`); (3) scan-activity Δ in each window — snapshots, files analyzed, defects introduced, defects eliminated (from `scan_activity_trend.json`); (4) **stream activity** — set diff on which streams have any activity in the recent-snapshot sample (from `snapshot_performance.json`); reports `newly active` and `went dark` streams only, not per-stream count deltas — the underlying metric is a top-N sample so count deltas from it are dominated by top-N sliding (Checkers going from 97 → 96 because one new snapshot on another stream evicted the oldest Checkers row is noise, not signal); (5) defects-by-project Δ + ranking movement (▲ climbed, ▼ dropped, `new` / `dropped` for one-sided rows). Security-focused deltas (OWASP / CWE) and cross-instance aggregated deltas are deliberately out of scope for v1.1.4 — the `delta_metrics.py` primitives (`diff_project_set` / `diff_numeric` / `diff_ranking`) are generic and will extend to those in a later release.
  - **Recommended workflow** — see the "Quarterly Comparison Workflow" section in `README.md` and the appended section in `EXPORT_QUICKSTART.md`: run `coverity-export --days 90 --anonymize --mapping-file archive/quarterly-mapping.json --output archive/2026-QN` at the end of every quarter with the **same** `--days` and `--mapping-file`, archive the ZIP, then compare with `coverity-delta --previous archive/2026-Q1/*.zip --current archive/2026-Q2/*.zip`. `--days` must match so windowed metrics (scan activity, cadence) remain comparable across quarters; the shared `--mapping-file` keeps `project_001` pointing at the same real project quarter after quarter. Both are enforced by the CLI's validation gate.
  - **Guardrails, all hard-fails**: (a) instance-list mismatch across ZIPs → refuse; (b) `--days` mismatch → refuse, with `--allow-window-mismatch` escape hatch documented as testing / advanced use only (report is emitted anyway with a `window_mismatch` entry in `delta.json.warnings`, ⚠ markers on affected sections in the HTML, and a `normalized_per_day` block alongside raw totals); (c) mismatched anonymization mappings (different `<zip>.mapping.json` fingerprints) → refuse with no override — fix by rerunning both exports with the same `--mapping-file`. Missing mapping on one side but not the other is downgraded to a warning so users trying delta without anonymization still get a report.
  - **HTML report** is self-contained (inline CSS, no external CDN dependencies, no Plotly): header banner with previous/current labels + window sizes + generated timestamp + warnings ribbon; tile cards for scalar deltas; sortable tables for per-project breakdowns using the repo's `data-table` autoloader convention. Deliberately terse — designed for a quarterly customer-adoption story, not a full dashboard rewrite.
  - Ships with 21 unit + end-to-end tests (`test_delta_metrics.py`, `test_delta_cli.py`) covering the diff primitives, `SnapshotLoader`, each compute function, the CLI's happy path, and every guardrail hard-fail + the escape hatch.
